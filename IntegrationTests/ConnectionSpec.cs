...
namespace IntegrationTests
{
	public class ConnectionSpec : IntegrationContext, IDisposable
	{
		...
		{
			var clientId = GetClientId();
			var clientFoo = await GetClientAsync();
			var clientBar = await GetClientAsync();
			Debugger.Break();

			List<char> events = new List<char>();
			

			server.ClientConnected += (sender, id) => events.Add('c');
			server.ClientDisconnected += (sender, id) => events.Add('d');
