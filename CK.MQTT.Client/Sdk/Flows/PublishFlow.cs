            ...
            using(var intervalSubscription = Observable
                ...
                .Subscribe( async _ =>
                {
                    if( channel.IsConnected )
                    {
                        ...
                        await channel.SendAsync( m, message: sentMessage );
                    }
                } ) )
            {
                await channel
               ...
               .OfType<IActivityMonitor, T, IActivityMonitor, IPacket>()
               .FirstOrDefaultAsync( x => x.Item2.PacketId == sentMessage.PacketId );
            }
