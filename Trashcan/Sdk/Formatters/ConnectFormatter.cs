        ...
        protected override Connect Read( byte[] bytes )
        {
            ...
            if( protocolLevel < MqttProtocol.SupportedLevel )
            {
                ...
                throw new MqttConnectionException(
                    ConnectionStatus.UnacceptableProtocolVersion,
            ...
            if( clientId.Length > MqttProtocol.ClientIdMaxLength )
                throw new MqttConnectionException( ConnectionStatus.IdentifierRejected, ClientProperties.ConnectFormatter_ClientIdMaxLengthExceeded );
            ...
            if( string.IsNullOrEmpty( clientId ) && !cleanSession )
                throw new MqttConnectionException( ConnectionStatus.IdentifierRejected, ClientProperties.ConnectFormatter_ClientIdEmptyRequiresCleanSession );

            if( string.IsNullOrEmpty( clientId ) )
            {
                clientId = MqttClientOld.GetAnonymousClientId();
            }
