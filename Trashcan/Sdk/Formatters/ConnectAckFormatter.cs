...
namespace CK.MQTT.Sdk.Formatters
{
    internal class ConnectAckFormatter : Formatter<ConnectAck>
    {
        ...
        protected override ConnectAck Read( byte[] bytes )
        {
            ...
            ConnectionStatus returnCode = (ConnectionStatus)bytes.Byte( connectAckFlagsIndex + 1 );
        ...
        {
            if( packet.Status != ConnectionStatus.Accepted && packet.SessionState )
                throw new MqttException( ClientProperties.ConnectAckFormatter_InvalidSessionPresentForErrorReturnCode );

            byte connectAckFlagsByte = Convert.ToByte( packet.SessionState );
