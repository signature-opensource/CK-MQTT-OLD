...
namespace Tests.Flows
{
    public class PublishSenderFlowSpec
    {
        ...
        public void when_sending_publish_with_qos1_and_publish_ack_is_not_received_then_publish_is_re_transmitted()
        {
            ...
            var flowTask = flow.SendPublishAsync( TestHelper.Monitor, clientId, publish, channel.Object );

            var retried = retrySignal.Wait( 2000 );

            Assert.True( retried );
            channel.Verify( c => c.SendAsync( TestHelper.Monitor, It.Is<IPacket>( p => p is Publish &&
        ...
        public void when_sending_publish_received_then_publish_release_is_sent()
        {
            ...
            var flowTask = flow.ExecuteAsync( TestHelper.Monitor, clientId, publishReceived, channel.Object );

            var ackSent = ackSentSignal.Wait( 2000 );

            Assert.True( ackSent );
            channel.Verify( c => c.SendAsync( TestHelper.Monitor, It.Is<IPacket>( p => p is PublishRelease
