...
namespace Tests.Flows
{
    ...
    public class PublishReceiverFlowSpec
    {
        ...
        public async Task when_sending_publish_with_qos0_then_publish_is_sent_to_subscribers_and_no_ack_is_sent()
        {
            ...
            Subject<Mon<IPacket>> client1Receiver = new Subject<Mon<IPacket>>();
            ...
            Subject<Mon<IPacket>> client2Receiver = new Subject<Mon<IPacket>>();
            ...
            Publish publish = new Publish( topic, Encoding.UTF8.GetBytes( "Publish Receiver Flow Test" ), MqttQualityOfService.AtMostOnce, retain: false, duplicated: false );
            ...
            publishSenderFlow.Verify( s => s.SendPublishAsync( TestHelper.Monitor, It.Is<string>( x => x == subscribedClientId1 ),
                It.Is<Publish>( p => p.Topic == publish.Topic &&
                    p.Payload.ToArray().SequenceEqual( publish.Payload.ToArray() ) ),//TODO: Spanify this.
        ...
        public async Task when_sending_publish_with_qos1_then_publish_is_sent_to_subscribers_and_publish_ack_is_sent()
        {
            ...
            Subject<Mon<IPacket>> clientReceiver = new Subject<Mon<IPacket>>();
            ...
            Publish publish = new Publish( topic, Encoding.UTF8.GetBytes( "Publish Flow Test" ), MqttQualityOfService.AtLeastOnce, retain: false, duplicated: false, packetId: packetId );
            ...
            channel.Verify( c => c.SendAsync( It.Is<Mon<IPacket>>( p => p.Item is PublishAck &&
        ...
        public async Task when_sending_publish_with_qos2_then_publish_is_sent_to_subscribers_and_publish_received_is_sent()
        {
            ...
            Publish publish = new Publish( topic, Encoding.UTF8.GetBytes( "Publish Flow Test" ), MqttQualityOfService.ExactlyOnce, retain: false, duplicated: false, packetId: packetId );
            ...
            receiver.OnNext( new Mon<IPacket>( TestHelper.Monitor, new PublishRelease( packetId.Value ) ) );
            ...
            publishSenderFlow.Verify( s => s.SendPublishAsync( TestHelper.Monitor, It.Is<string>( x => x == subscribedClientId ),
                ...
                It.Is<IMqttChannel<IPacket>>( c => c == clientChannel.Object ), It.Is<PendingMessageStatus>( x => x == PendingMessageStatus.PendingToSend ) ) );
            retainedRepository.Verify( r => r.Create( It.IsAny<RetainedMessage>() ), Times.Never );
            channelMock.Verify( c => c.SendAsync( It.Is<Mon<IPacket>>( p => p.Item is PublishReceived && (p.Item as PublishReceived).PacketId == packetId.Value ) ) );
        }
        ...
        public void when_sending_publish_with_qos2_and_no_release_is_sent_after_receiving_publish_received_then_publish_received_is_re_transmitted()
        {
            ...
            Assert.True( retried );
            channel.Verify( c => c.SendAsync( It.Is<Mon<IPacket>>( p => p.Item is PublishReceived
        ...
        public async Task when_sending_publish_with_retain_then_retain_message_is_created()
        {
            ...
            Publish publish = new Publish( topic, Encoding.UTF8.GetBytes( payload ), qos, retain: true, duplicated: false );
            ...
            retainedRepository.Verify( r => r.Create( It.Is<RetainedMessage>( m => m.Id == topic && m.QualityOfService == qos && m.Payload.ToArray().SequenceEqual( publish.Payload.ToArray() ) ) ) );
            channel.Verify( c => c.SendAsync( It.IsAny<Mon<IPacket>>() ), Times.Never );
        }
        ...
        public async Task when_sending_publish_with_qos_higher_than_supported_then_supported_is_used()
        {
            ...
            publishSenderFlow.Verify( s => s.SendPublishAsync( TestHelper.Monitor, It.Is<string>( x => x == subscribedClientId ),
                ...
                It.Is<IMqttChannel<IPacket>>( c => c == clientChannel.Object ), It.Is<PendingMessageStatus>( x => x == PendingMessageStatus.PendingToSend ) ) );
            retainedRepository.Verify( r => r.Create( It.IsAny<RetainedMessage>() ), Times.Never );
            channel.Verify( c => c.SendAsync( It.Is<Mon<IPacket>>( p => p.Item is PublishAck && (p.Item as PublishAck).PacketId == packetId.Value ) ) );
        }
        ...
        public void when_sending_publish_with_qos_higher_than_zero_and_without_packet_id_then_fails()
        {
            ...
            Publish publish = new Publish( topic, Encoding.UTF8.GetBytes( "Publish Flow Test" ), MqttQualityOfService.AtLeastOnce, retain: false, duplicated: false );
        ...
        public async Task when_sending_publish_and_subscriber_with_qos1_send_publish_ack_then_publish_is_not_re_transmitted()
        {
            ...
            Subject<Mon<IPacket>> clientReceiver = new Subject<Mon<IPacket>>();
            Subject<Mon<IPacket>> clientSender = new Subject<Mon<IPacket>>();
            Mock<IMqttChannel<IPacket>> clientChannel = new Mock<IMqttChannel<IPacket>>();

            clientSender.OfMonitoredType<Publish, IPacket>().Subscribe( p =>
           {
               clientReceiver.OnNext( new Mon<IPacket>( TestHelper.Monitor, new PublishAck( p.Item.PacketId.Value ) ) );
           } );
            ...
            clientChannel.Verify( c => c.SendAsync( It.Is<Mon<IPacket>>( p => p.Item is Publish ) ), Times.Never );
        }
        ...
        public async Task when_sending_publish_and_subscriber_with_qos2_send_publish_received_then_publish_is_not_re_transmitted()
        {
            ...
            clientSender.OfMonitoredType<Publish, IPacket>().Subscribe( p =>
            {
                clientReceiver.OnNext( new Mon<IPacket>( TestHelper.Monitor, new PublishReceived( p.Item.PacketId.Value ) ) );
            } );
        ...
        public async Task when_sending_publish_release_then_publish_complete_is_sent()
        {
            ...
            channel.Verify( c => c.SendAsync( It.Is<Mon<IPacket>>( p => p.Item is PublishComplete && (p.Item as PublishComplete).PacketId == packetId ) ) );
        }
        ...
        public void when_sending_publish_to_a_system_topic_with_remote_client_then_fails()
        {
            ...
            Publish publish = new Publish( systemTopic,Encoding.UTF8.GetBytes( "Publish Receiver Flow Test" ), MqttQualityOfService.AtMostOnce, retain: false, duplicated: false );
