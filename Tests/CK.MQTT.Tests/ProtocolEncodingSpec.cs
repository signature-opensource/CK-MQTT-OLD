...
namespace Tests
{
	public class ProtocolEncodingSpec
	{
		...
		{
			var text = "Foo";
			var encoded = MqttConstants.Encoding.EncodeString (text);

			(MqttConstants.StringPrefixLength + text.Length).Should().Be(encoded.Length);
		...
		public void when_encoding_string_with_exceeded_length_then_fails()
		{
			...
			Assert.Throws<MqttException>(() => MqttConstants.Encoding.EncodeString (text));
		}
		...
		public void when_encoding_int32_minor_than_max_protocol_length_then_is_encoded_big_endian()
		{
			...
			var encoded = MqttConstants.Encoding.EncodeInteger (number);
		...
		public void when_encoding_int32_major_than_max_protocol_length_then_fails()
		{
			...
			Assert.Throws<MqttException>(() => MqttConstants.Encoding.EncodeInteger (number));
		}
		...
		public void when_encoding_remaining_length_then_succeeds()
		{
			...
			var encoded1From = MqttConstants.Encoding.EncodeRemainingLength (length1From);
			var encoded1To = MqttConstants.Encoding.EncodeRemainingLength (length1To);

			var encoded2From = MqttConstants.Encoding.EncodeRemainingLength (length2From);
			var encoded2To = MqttConstants.Encoding.EncodeRemainingLength (length2To);

			var encoded3From = MqttConstants.Encoding.EncodeRemainingLength (length3From);
			var encoded3To = MqttConstants.Encoding.EncodeRemainingLength (length3To);

			var encoded4From = MqttConstants.Encoding.EncodeRemainingLength (length4From);
			var encoded4To = MqttConstants.Encoding.EncodeRemainingLength (length4To);

			var encoded5 = MqttConstants.Encoding.EncodeRemainingLength (length5); //0x40
			var encoded6 = MqttConstants.Encoding.EncodeRemainingLength (length6); //193 2
		...
		public void when_decoding_remaining_length_then_succeeds()
		{
            ...
            var length1From = MqttConstants.Encoding.DecodeRemainingLength (encoded1From.ToArray(), out var arrayLength1From); //0
            var length1To = MqttConstants.Encoding.DecodeRemainingLength (encoded1To.ToArray(), out var arrayLength1To); //127

            var length2From = MqttConstants.Encoding.DecodeRemainingLength (encoded2From.ToArray(), out var arrayLength2From); //128
            var length2To = MqttConstants.Encoding.DecodeRemainingLength (encoded2To.ToArray(), out var arrayLength2To); //16383

            var length3From = MqttConstants.Encoding.DecodeRemainingLength (encoded3From.ToArray(), out var arrayLength3From); //16384
            var length3To = MqttConstants.Encoding.DecodeRemainingLength (encoded3To.ToArray(), out var arrayLength3To); //2097151

            var length4From = MqttConstants.Encoding.DecodeRemainingLength (encoded4From.ToArray(), out var arrayLength4From); //2097152
            var length4To = MqttConstants.Encoding.DecodeRemainingLength (encoded4To.ToArray(), out var arrayLength4To); //268435455

            var length1 = MqttConstants.Encoding.DecodeRemainingLength (bytes1.ToArray(), out var arrayLength1); //64
            var length2 = MqttConstants.Encoding.DecodeRemainingLength (bytes2.ToArray(), out var arrayLength2); //321
		...
		public void when_decoding_malformed_remaining_length_then_fails()
		{
            ...
            //According to spec samples: http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html#_Toc385349213
            Assert.Throws<MqttException> (() => MqttConstants.Encoding.DecodeRemainingLength (bytes.ToArray (), out _));
		}
