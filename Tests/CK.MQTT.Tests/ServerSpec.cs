...
namespace Tests
{
    public class ServerSpec
    {
        ...
        public void when_server_does_not_start_then_connections_are_ignored()
        {
            ...
            Subject<IMonitored<IPacket>> packets = new Subject<IMonitored<IPacket>>();
            ...
            packetChannel
                ...
                .Returns( new Subject<IMonitored<IPacket>>() );
            packetChannel
            ...
            sockets.OnNext( Mock.Of<IMqttChannel<byte[]>>( x => x.ReceiverStream == new Subject<IMonitored<byte[]>>() ) );
            sockets.OnNext( Mock.Of<IMqttChannel<byte[]>>( x => x.ReceiverStream == new Subject<IMonitored<byte[]>>() ) );
            sockets.OnNext( Mock.Of<IMqttChannel<byte[]>>( x => x.ReceiverStream == new Subject<IMonitored<byte[]>>() ) );
